...
import {
  bookTourForUserApi,
  getBookedTourForUserApi,
} from '../../api/user/book_tour.api';
...
const validationTourist = yup.object().shape({
  fullName: yup
    ...
    .matches(CUSTOM_REGEX.REGEX_STRING, 'Invalid fullName'),
  birthDate: yup.date().required('Required birthDate'),
  gender: yup
    .string()
    .required('Required gender')
    .matches(CUSTOM_REGEX.REGEX_STRING, 'Invalid gender'),
});
...
const validationBooking = yup.object().shape({
  ...
  email: yup
    ...
    .matches(CUSTOM_REGEX.EMAIL, 'Invalid email!'),
  phone: yup
    .string()
    .required('Required phone')
    .matches(CUSTOM_REGEX.PHONE, 'Invalid phone!'),
  address: yup
    .string()
    .required('Required address')
    .matches(CUSTOM_REGEX.REGEX_STRING, 'Invalid address!'),
  adultNumber: yup
    .number()
    .required('Required adultNumber')
    .min(1, 'Adult number must be greater than 1!'),
  childrenNumber: yup
    .number()
    .required('Required childrenNumber')
    .min(0, 'Children number must be greater than 0!'),
  babyNumber: yup
    .number()
    .required('Required babyNumber')
    .min(0, 'Baby number must be greater than 0!'),
  note: yup.string(),
  tourId: yup.number(),
  touristList: yup
    .array()
    .of(validationTourist)
    .test(
      'Tourist-list-length',
      'Tourist List length must be equal to the sum of adultNumber, childrenNumber and babyNumber',
      function (value) {
  ...
  adults: yup
    ...
    .test(
      'adults-length',
      'Adults length must be equal to adultNumber!',
      function (value) {
  ...
  children: yup
    ...
    .test(
      'children-length',
      'Children length must be equal to childrenNumber!',
      function (value) {
  ...
  babies: yup
    ...
    .test(
      'babies-length',
      'Babies length must be equal to babyNumber!',
      function (value) {
