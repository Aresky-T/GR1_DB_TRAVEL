...
const TouristAttractionCreateContainer = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: values => {
            validateTouristAttraction.validate(values, {abortEarly: true})
                .then(data => {
                    console.log(data)
                    createTouristAttractionApi(data, account.accessToken)
                        .then(res => {
                            successAlert("Thành công", "Địa điểm du lịch mới đã được tạo thành công!", "OK")
                                .then(result => {
                        ...
                        .catch(err => {
                            errorAlert('Thất bại', "Tạo mới không thành công, vui lòng thử lại!")
                            console.log(err)
                        })
                })
                .catch(err => {
