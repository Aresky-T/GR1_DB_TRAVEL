...
const TourGuideCreateContainer = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: values => {
            validateTourGuide.validate(values)
                .then(data => {
                    dispatch(onLoading());
                    account.accessToken && createTourGuideApi(data, account.accessToken)
                        .then(res => {
                            dispatch(offLoading());
                            successAlert("Thành công", "Đã thêm mới", "OK")
                                .then(result => {
                        ...
                        .catch(err => {
                            console.log(err)
                            dispatch(offLoading());
                            errorAlert("Thất bại", "Thêm mới không thành công, hãy kiểm tra lại!", "OK")
                        })
                })
                .catch(err => {
                    console.log(err)
