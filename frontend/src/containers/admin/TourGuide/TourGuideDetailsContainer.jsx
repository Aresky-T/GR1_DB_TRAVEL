...
import {deleteTourGuideApi, getTourGuideDetailApi, updateTourGuideApi} from '../../../api/admin/tour_guide.api';
...
const TourGuideDetailsContainer = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: values => {
            ...
            validateTourGuide.isValid(values)
                .then(isValid => {
                    dispatch(onLoading());
    ...
    const handleDeleteTourGuide = () => {
        const id = param.id;
        questionAlert("Yêu cầu xác nhận", "Tất cả dữ liệu về Nhân viên này sẽ bị xóa đi, bạn có chắc chắn muốn xóa không?", "Tôi chắc chắn, Xóa!", "Hủy bỏ")
            .then(result => {
                if (result.isConfirmed) {
                    deleteTourGuideApi(id, account.accessToken)
                        .then(res => {
                            successAlert('Thành công', 'Đã xóa nhân viên thành công', 'OK')
                                .then(result => {
                                    if (result.isConfirmed) {
