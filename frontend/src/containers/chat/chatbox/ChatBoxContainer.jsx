...
import { authSelector, useChat } from "../../../redux/selector";
import { useEffect, useState } from "react";
import ChatBoxBody from "../../../components/chat/body/ChatBoxBody";
import toast from "react-hot-toast";
import ValidateUtils from "../../../utils/validate";
import { connectEmployeeForGuestCustomerApi, connectEmployeeForRegisteredCustomerApi } from "../../../api/chat/chat.api";
import { getStompClient2 } from "../../../config/webSocketConfig";
import { onCancelChat, updateChatField, updateChatFields, updateChatFormField, updateChatStatusField, updateChatStatusMultipartFields } from "../../../redux/slices/chat.slice";
...
const ChatBoxContainer = ({ isShowChat, handleHiddenChat }) => {
        ...
        switch (chat.type) {
                    ...
                    getStompClient2()
                        .then(client => {
                            ...
                            setTimeout(() => {
                                ...
                                dispatch(updateChatStatusMultipartFields({
                                    isConnecting: false,
                                    isConnected: true,
                                }));
            ...
            case "REGISTERED":
                if (account.accessToken) {
                    try {
                        dispatch(updateChatStatusField({ key: "isConnecting", value: true }))
                        const res = await connectEmployeeForRegisteredCustomerApi(account.accessToken);
                        const data = res.data;
                        const employeeId = data.employee.id;
                        ...
                        setTimeout(() => {
                            toast.error("Kết nối thất bại!")
                            dispatch(updateChatStatusMultipartFields({
                                isConnected: false, isConnecting: false, type: null
                            }));
                        }, 500)
                    }
                } else {
    ...
    async function handleSubmitSendMessage(e) {
        ...
        try {
            ...
            setTimeout(() => {
                ...
                // })
                client.publish({ destination: `/app/chat/refresh.conversation.for.customer/${id}` });
    ...
    useEffect(() => {
        if (id) {
            getStompClient2()
                .then((client) => {
                    client.subscribe(`/topic/customer/chat/conversation/${id}`, (message) => {
    ...
    useEffect(() => {
        if (employee.id) {
            getStompClient2((client) => {
                client.subscribe(`/topic/chat/employee-status/${employee.id}`, (message) => {
